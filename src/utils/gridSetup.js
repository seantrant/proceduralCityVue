export class GridSetup {
  ...
  createGridArray() {
    const arrayContainingGrids = [];
    let currentRow = 0;
    let currentColumn = 0;
    const { gridSize } = this.getGridConfig(); // must have square root for now
    const totalGrids = gridSize * gridSize;
    let count = 1;
    for (let i = 0; i < totalGrids; i++) {
      const grid = { id: i, coords: { x: currentRow, y: currentColumn }, contents: null }; // makde class for grid
      if (count == gridSize - 1) {
        count = 1;
      }
