...
export function animateSceneFrame(vm, now, options = {}) {
  ...
  const skyDome = vm.scene.getObjectByName && vm.scene.getObjectByName(vm.skyDomeName)
  if (skyDome && skyDome.position && vm.camera && vm.camera.position) {
    skyDome.position.copy(vm.camera.position)
  }

  const roofLightGroup = vm.scene.getObjectByName && vm.scene.getObjectByName('roofLightGroup')
